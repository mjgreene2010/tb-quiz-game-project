...
function setupQuestionsAndAnswers() {
  ...
  let question4 = {
    id: "quest3",
    question:
      "Use the _____ property to determine the width of the input field:",
    answer1: "width",
    answer2: "height",
    answer3: "margin",
    answer4: null,
...
function gradeQuestion(questionId, answerNumber) {
  ...
  for (let i = 0; i < questionArr.length; i++) {
    if (
      questionId == questionArr[i].id &&
      answerNumber == questionArr[i].correctAnswer
    ) {
      isCorrect = true;
      break;
    }
...
function evaluateQuestion(element, questNum) {
  ...
  if (parseInt(localStorage.getItem(timerKey)) > 0) {
    ...
    if (answer) {
      ...
      alert("Correct");
      console.log(
        "evaluateQuestion localStorage.getItem(myScoreKey) ",
        localStorage.getItem(myScoreKey)
      );
    } else {
...
//   console.log("countDowntime: ", countDownTimer);
// }
...
//   countDownTimer--;
//   console.log("updateTimer: countDownTier", countDownTimer);
//   presentTimer();
// }
...
// function showScoreBoard() {
//   let scoreBoard = document.getElementById("score-board");
//   console.log("hsowScoreboard: scoreBaord ", scoreBoard);
...
//   let qSpan = document.createElement("span");
//   qSpan.innerHTML = arrayOfQuestions[questionIndex].question;
